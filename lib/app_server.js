...
function handleLocationChange(socket, users, room) {
  socket.on("locationChange", function(position) {
    ...
    for(var userId in users){
      ...
      //the users that are new to this user
      var newConnections = nearByUserIds.diff(users[userId].nearByUserIds);
      for(var i = 0; i < newConnections.length; i++) {
        if(userId != socket.id){ //don't display all users to the new users as a bunch of "new connections"
          io.sockets.sockets[userId].emit("message", {
            user: users[newConnections[i]].name,
            text: " is now in your circle.",
            system: true
          });
        }
      }
  ...
  socket.on("nameChange", function(name) {
    ...
    for(var i = 0; i < users[socket.id].nearByUserIds.length; i++) {
      ...
      io.sockets.sockets[nearByUserId].emit("message", {
        user: previousName,
        text: " is now know as <strong>" + name + "</strong>.",
        system: true
      });
