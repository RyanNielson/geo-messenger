...
function getUsersWithinRadius(local_user, local_user_id) {
  var localUsers = {};
  if(typeof local_user.location !== 'undefined') {
    for(var index in users){
      if(index != local_user_id) {
        var user = users[index];
        if(typeof user.location !== 'undefined'){
          var diff = calculateDistance(user.location, local_user.location);
          if(diff <= proximity)
